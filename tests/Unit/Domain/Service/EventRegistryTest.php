...
class EventRegistryTest extends TestCase
{
    ...
    public function testExistsReturnsTrueForAllKnownEvents(): void
    {
        foreach ($this->allEventTypes as $eventType) {
            $this->assertTrue(
                $this->registry->exists($eventType),
    ...
    public function testExistsReturnsFalseForUnknownEvent(): void
    {
        ...
        $this->assertFalse($this->registry->exists(''));
    }
    ...
    {
        foreach ($this->allEventTypes as $eventType) {
            $subject = $this->registry->buildSubject($eventType, 'TEST-001');
    ...
    public function testSevenEventsExist(): void
    {
        ...
        foreach ($this->allEventTypes as $eventType) {
            $this->assertTrue($this->registry->exists($eventType));
        }
    }

    public function testAllReturnsAllEvents(): void
    {
        $all = $this->registry->all();
        $this->assertCount(7, $all);
        foreach ($this->allEventTypes as $eventType) {
            $this->assertArrayHasKey($eventType, $all);
        }
    }
}
