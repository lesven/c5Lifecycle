...
class CustomFieldMapperTest extends TestCase
{
    ...
    {
        $this->mapper = new CustomFieldMapper();
    }
    ...
    public function testRzRetireMapsFields(): void
    {
        ...
        $result = $this->mapper->map('rz_retire', $data);
    ...
    public function testAdminProvisionMapsFields(): void
    {
        ...
        $result = $this->mapper->map('admin_provision', $data);
    ...
    {
        $this->assertEmpty($this->mapper->map('admin_return', ['admin_user' => 'admin1']));
        $this->assertEmpty($this->mapper->map('rz_owner_confirm', ['owner' => 'Team X']));
    }
    ...
    {
        $this->assertTrue($this->mapper->hasMapping('rz_provision'));
        $this->assertTrue($this->mapper->hasMapping('rz_retire'));
        $this->assertTrue($this->mapper->hasMapping('admin_provision'));
    }

    public function testHasMappingReturnsFalseForUnmappedEvents(): void
    {
        $this->assertFalse($this->mapper->hasMapping('admin_return'));
        $this->assertFalse($this->mapper->hasMapping('rz_owner_confirm'));
